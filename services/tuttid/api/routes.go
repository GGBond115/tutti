...
func registerConnectorMarketRoutes(mux *http.ServeMux, wrapper *tuttigenerated.ServerInterfaceWrapper) {
	...
	mux.HandleFunc("/v1/connector-market/connectors/{connectorKey}/runtime:restart", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			tuttitypes.WriteMethodNotAllowed(w)
			return
		}
		wrapper.RestartConnectorMarketRuntime(w, r)
	})
