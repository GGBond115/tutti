...
export function areAgentSessionReplayPlaybackSnapshotsEqual(
  ...
  return (
    ...
    current.playback.playbackElapsedMs === next.playback.playbackElapsedMs &&
    ...
    current.status.errorCause?.code === next.status.errorCause?.code &&
    current.status.errorCause?.message === next.status.errorCause?.message &&
    ...
    current.status.totalDurationMs === next.status.totalDurationMs &&
