...
The sync command requires an authorized local service checkout. CI and local
checks use the vendored files and do not require network access:
...
path, and request authorization callback. Successful response bodies are
bounded to 8 MiB and decoded with additive protobuf JSON fields ignored.
