...
// ValidateUniquePermissions validates a complete manifest permission list.
func ValidateUniquePermissions(values []string) error {
	return validateUniquePermissions(values)
}
...
// IsSafeRelativeEntrypoint reports whether value is a normalized relative
// artifact path that cannot escape its prepared root.
func IsSafeRelativeEntrypoint(value string) bool {
	return safeRelativeEntrypoint(value)
}

// IsArtifactSHA256 reports whether value is a canonical lowercase SHA-256.
func IsArtifactSHA256(value string) bool {
	return artifactSHA256Pattern.MatchString(value)
}
