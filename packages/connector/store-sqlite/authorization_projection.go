...
WHERE account_id = ? AND connector_key = ?`, accountID, connectorKey).Scan(&payload); err != nil {
		return contracts.AuthorizationProjection{}, mapNotFound(err)
	}
	var projection contracts.AuthorizationProjection
	if err := json.Unmarshal([]byte(payload), &projection); err != nil {
		return contracts.AuthorizationProjection{}, fmt.Errorf("decode connector authorization projection: %w", err)
	}
...
WHERE account_id = ? AND connector_key = ?`, projection.AccountID, projection.ConnectorKey).Scan(&currentPayload)
	if currentErr == nil {
		var current contracts.AuthorizationProjection
...
func (store *Store) ApplyAuthorizationSnapshot(
	ctx context.Context,
	accountID string,
	snapshot contracts.AuthorizationSnapshot,
) (contracts.AuthorizationSnapshotApplyResult, error) {
	accountID = strings.TrimSpace(accountID)
	if accountID == "" {
		return contracts.AuthorizationSnapshotApplyResult{}, errors.New("authorization snapshot account is required")
	}
	tx, err := store.db.BeginTx(ctx, nil)
	if err != nil {
		return contracts.AuthorizationSnapshotApplyResult{}, err
	}
	defer func() { _ = tx.Rollback() }()
	...
	existing := make(map[string]contracts.AuthorizationProjection)
	for rows.Next() {
		...
		if err := json.Unmarshal([]byte(payload), &projection); err != nil {
			_ = rows.Close()
			return contracts.AuthorizationSnapshotApplyResult{}, fmt.Errorf("decode connector authorization projection: %w", err)
		}
	...
	now := time.Now().UTC()
	result := contracts.AuthorizationSnapshotApplyResult{}
	effective := existing
	if !hasRevision || snapshot.Revision > currentRevision {
		incoming := make(map[string]contracts.AuthorizationProjection, len(snapshot.Connectors))
		for _, projection := range snapshot.Connectors {
			...
			if projection.ConnectorKey == "" {
				return contracts.AuthorizationSnapshotApplyResult{}, errors.New("authorization snapshot contains an empty connector key")
			}
		...
		for connectorKey, previous := range existing {
			...
			incoming[connectorKey] = contracts.AuthorizationProjection{
				AccountID: accountID, ConnectorKey: connectorKey, ServerRevision: snapshot.Revision,
				ServerSynchronized: true, State: contracts.AuthorizationStateDisconnected, UpdatedAt: now,
			}
		}
		...
		for connectorKey, projection := range incoming {
			...
			payload, err := json.Marshal(projection)
			if err != nil {
				return contracts.AuthorizationSnapshotApplyResult{}, fmt.Errorf("encode connector authorization projection: %w", err)
			}
...
func samePublicAuthorizationProjection(left, right contracts.AuthorizationProjection) bool {
...
func bumpAuthorizationProjectionRevisionOn(ctx context.Context, tx *sql.Tx, connectorKey string) error {
	...
	eventPayload, err := json.Marshal(contracts.ChangedEvent{ConnectorKey: connectorKey, Revision: revision})
	if err != nil {
		return err
	}
...
func connectedAuthorizationReceiptConnectorKeys(
	...
	tx *sql.Tx,
	accountID string,
	projections map[string]contracts.AuthorizationProjection,
) ([]string, error) {
	connected := make(map[string]struct{})
	for connectorKey, projection := range projections {
		if projection.ServerSynchronized && projection.State == contracts.AuthorizationStateConnected {
