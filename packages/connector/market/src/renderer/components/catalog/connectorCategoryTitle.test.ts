...
test("changes locale presentation without requesting the Market again", async () => {
  ...
  try {
    ...
    const surfaceSnapshot = { market: {}, ui: {}, view: viewState };
    const rendererModel = {
      commands: {
        async loadMore() {
          marketRequests += 1;
    ...
    await renderCatalog(reactRoot, rendererModel, "en-US");
    assert.match(container.textContent ?? "", /Developer Tools/u);

    await renderCatalog(reactRoot, rendererModel, "zh-CN");
...
async function renderCatalog(
  ...
  await act(async () => {
    reactRoot.render(
      createElement(ConnectorMarketRendererProvider, {
