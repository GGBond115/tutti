...
import "github.com/tutti-os/tutti/packages/connector/contracts"
...
func (registry *RouteRegistry) ConnectorSummaries() []contracts.ConnectorSummary {
	routes := registry.Routes()
	connectors := make([]contracts.ConnectorSummary, 0, len(routes))
...
func connectorSummaryFromDescriptor(route RouteDescriptor) contracts.ConnectorSummary {
	interfaces := make([]contracts.ConnectorInterfaceSummary, 0, 2)
	if route.HasMCP {
		interfaces = append(interfaces, contracts.ConnectorInterfaceSummary{Kind: "mcp", ServerName: "connector",
			ToolPrefix: route.ConnectorKey + "_", Status: string(route.InterfaceState("mcp"))})
	}
	if route.CLIInvocationCommand != "" {
		interfaces = append(interfaces, contracts.ConnectorInterfaceSummary{Kind: "cli", Command: route.CLIInvocationCommand,
			Status: string(route.InterfaceState("cli"))})
	}
	return contracts.ConnectorSummary{Key: route.ConnectorKey, Version: route.ConnectorVersion, Name: route.DisplayName, Description: route.Description,
		Skills: append([]contracts.ConnectorSkillSummary(nil), route.Skills...), Interfaces: interfaces}
}
