...
Make the delegation decision early. For a task expected to take more than a few minutes and spanning implementation plus non-trivial tests, compatibility analysis, or verification, default to at least two focused Luna workers before main-thread inspection: typically one for source behavior and implementation semantics, and one for tests, verification, and compatibility risks. Add a third worker only for another genuinely independent, non-trivial unit. Use one worker only when the scopes cannot be separated or the additional units are individually trivial. Spawn one worker per unit without forking the main conversation history; use the no-history option exposed by the current tool. Run read-only or isolated-worktree units in parallel. If write scopes cannot be isolated, run them sequentially. Each delegation must be self-contained and state the relevant context and files, boundaries, allowed state-changing actions, acceptance criteria, retry limit, and expected evidence.

Start workers before doing equivalent work in the main thread. Until their results arrive, do not inspect or implement the questions or files assigned to them. Continue only with non-overlapping work, or wait when no such work remains. After receiving their evidence, independently verify it before use.
