...
func projectRuntimeConnectorPromptContent(content []PromptContentBlock) []PromptContentBlock {
	...
	instruction := fmt.Sprintf(
		"Selected local connector(s): %s. For this request, use only these installed Tutti connectors for their corresponding external services. Follow the injected Connectors policy and its exact rendered commands: query `connector available`, select from its Skill summaries (fall back to `connector skills` for an older host), read the relevant connector Skill through `connector skill read`, resolve the canonical capability with `connector capabilities`, then invoke only through `connector invoke`. Never read or run a similarly named user-global or provider-native Skill, executable, MCP server, connector, or direct service CLI.",
