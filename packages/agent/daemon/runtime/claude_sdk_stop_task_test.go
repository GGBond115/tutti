...
func TestClaudeCodeSDKAdapterCancelTargetsStopsChildrenWhenRootSettlesDuringCancel(t *testing.T) {
	adapter := NewClaudeCodeSDKAdapter(nil)
	canceled := false
	stopped := true
	...
	adapter.beginClaudeSDKRootTurn(adapterSession, "turn-root-race", "")
	adapter.registerClaudeSDKTurn(adapterSession, "turn-root-race", nil)
	child, _, _, ok := adapterSession.updateClaudeSDKChild(session, claudeSDKChildUpdate{
		Key:             "toolu-agent-race",
		ParentToolUseID: "toolu-agent-race",
		RootTurnID:      "turn-root-race",
		TaskID:          "task-race",
		Async:           true,
		Started:         true,
	})
	...
	result, err := adapter.CancelTargets(context.Background(), session, []CancelTarget{
		{AgentSessionID: session.AgentSessionID, TurnID: "turn-root-race"},
		{AgentSessionID: child.AgentSessionID, TurnID: child.TurnID},
	}, "user_request")
	if err != nil {
		t.Fatalf("CancelTargets: %v", err)
	...
	if len(result.ConfirmedTargets) != 1 || result.ConfirmedTargets[0].AgentSessionID != child.AgentSessionID {
		t.Fatalf("confirmed targets = %#v, want only the explicitly stopped child", result.ConfirmedTargets)
	}
	sent := conn.sentRequests()
	if len(sent) != 2 || sent[0].Type != "cancel" || sent[1].Type != "stop_task" {
		t.Fatalf("sent requests = %#v, want root cancel followed by child stop_task", sent)
	}
}
